...
jq -n --slurpfile result "$result" --slurpfile failures "$scratch/classified.jsonl" --arg container "$container" --arg task "$task" --argjson complete "$logs_complete" --argjson terminal "$terminal_observed" '{taskArn:$task,logsComplete:$complete,terminalEventObserved:$terminal,taskStatus:($result[0].tasks[0].lastStatus | if . == "RUNNING" or . == "PENDING" or . == "STOPPED" then . else "UNKNOWN" end),recoveryComplete:($result[0].tasks[0].lastStatus == "STOPPED" and $complete and $terminal),taskStopped:($result[0].tasks[0].lastStatus == "STOPPED"),exitCode:([$result[0].tasks[0].containers[] | select(.name == $container) | .exitCode][0] // null),failureCategories:($failures | unique)}' > reconciliation-diagnostics.json
