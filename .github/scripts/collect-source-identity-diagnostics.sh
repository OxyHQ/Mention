...
jq -n --slurpfile result "$result" --slurpfile failures "$scratch/classified.jsonl" --arg container "$container" --arg task "$task" --argjson complete "$logs_complete" '{taskArn:$task,logsComplete:$complete,taskStopped:($result[0].tasks[0].lastStatus == "STOPPED"),exitCode:([$result[0].tasks[0].containers[] | select(.name == $container) | .exitCode][0] // null),failureCategories:($failures | unique)}' > reconciliation-diagnostics.json
