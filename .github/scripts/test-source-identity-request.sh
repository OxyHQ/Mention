...
for TEST_CASE in success custom-id wrong-image wrong-selector wrong-window; do
  export TEST_CASE REQUEST_INSPECTION=$selector
  if [[ "$TEST_CASE" == custom-id ]]; then export REQUEST_INSPECTION="${selector/a5956d3d-c89f-4c8a-b8c5-5abce3ac7c10/request-safe-123}"; fi
  ...
  if [[ "$TEST_CASE" == success || "$TEST_CASE" == custom-id ]]; then
