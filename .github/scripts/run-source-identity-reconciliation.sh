...
case "$DRY_RUN" in
  ...
  false)
    ...
    jq -e --arg sha "$DEPLOY_SHA" --arg digest "$EXPECTED_IMAGE_DIGEST" '.sourceSha == $sha and .imageDigest == $digest and .dryRun == true and .exitCode == 0 and .report.actorsExamined >= 0 and .report.postsExamined >= 0' reviewed-preview/reconciliation-report.json >/dev/null
    ;;
  *) exit 1 ;;
esac
...
jq -n --arg name "$container" --arg dry "$DRY_RUN" '{containerOverrides:[{name:$name,command:["busybox","timeout","-s","TERM","-k","30","3300","bun","packages/backend/dist/src/scripts/reconcileMetaIdentityAndCrossposts.js"],environment:[{name:"DRY_RUN",value:$dry},{name:"CONFIRM_ADMIN_MUTATION",value:(if $dry == "false" then "reconcileMetaIdentityAndCrossposts" else "" end)}]}]}' > "$work_dir/overrides.json"
...
for ((page=0; page<10000; page++)); do
  ...
  aws "${args[@]}" > "$work_dir/logs.json"
  jq -c '.events[].message | fromjson? | select(.msg == "[reconcileMetaIdentityAndCrossposts] complete")' "$work_dir/logs.json" >> "$work_dir/messages.jsonl"
...
# Preserve counts only, never raw logs, task definitions or inherited secret configuration.
jq -s -e --arg sha "$DEPLOY_SHA" --arg digest "$EXPECTED_IMAGE_DIGEST" --argjson dry "$DRY_RUN" --argjson code "$exit_code" '
  ...
  | {sourceSha:$sha,imageDigest:$digest,dryRun:$dry,exitCode:$code,report:{actorsExamined,actorsChanged,postsChanged,authorshipConflicts,mutesPreserved,clustersDissolved,postsExamined,postClustersCreated,refused}}
