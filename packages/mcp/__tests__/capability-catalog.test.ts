...
describe("Mention canonical capability catalog", () => {
  test("is valid, complete and has one policy for every tool", () => {
    ...
    expect(MENTION_CAPABILITY_CATALOG.tools).toHaveLength(71);
    ...
    expect(MENTION_TOOL_REGISTRY.definitions()).toHaveLength(71);
    expect(new Set(MENTION_CAPABILITY_CATALOG.tools.map((tool) => tool.name)).size).toBe(71);
  });
