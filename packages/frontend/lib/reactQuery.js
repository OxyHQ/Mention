...
exports.invalidateRelatedQueries = invalidateRelatedQueries;
 ...
 */
function invalidateRelatedQueries(queryClient, keyPattern) {
    // Remove the first element (base type), keep other filtering criteria
    var filterKey = keyPattern.slice(1);
    ...
    return queryClient.invalidateQueries({
        predicate: function (query) {
            ...
            return filterKey.length === 0 ||
                filterKey.every(function (key, index) {
                    return key === undefined || key === query.queryKey[index + 1];
                });
        },
    });
}
