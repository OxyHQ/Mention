 ...
 * place themselves in from the pips at a glance; it is also as many taps as it can
 * take to reach any post from any other, since the advance control only goes forward.
 *
 * Five pips and a 48dp advance control fit on the byline row of the narrowest card
 * that carries them — 250dp, the default placement — with room left for the author's
 * name. A longer rotation would take that room, one pip and one gap at a time.
 */
...
/**
 * The position to STORE when the rotation advances, given what the store holds RIGHT NOW.
 *
 * Both arguments are the store's own values, read inside its transaction, and that is the
 * whole point of the function: the LENGTH comes out of the encoded blob rather than from
 * the caller, so an advance is never computed against a rotation that has already been
 * replaced. Two callers ask for this — the refresh tick and the reader's tap — and a tap
 * that lands in the same moment as a fetch which SHORTENED the rotation still moves to a
 * position that exists in the new one.
 *
 * A missing index is the first-run store, where nothing has been written yet, and counts
 * as position zero rather than as a reason to leave the cursor alone.
 */
internal fun advancedRotationIndex(storedPosts: String?, storedIndex: Int?): Int =
    nextRotationIndex(storedIndex ?: 0, decodePosts(storedPosts).size)
