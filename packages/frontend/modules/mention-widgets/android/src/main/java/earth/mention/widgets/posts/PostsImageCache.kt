 ...
 *
 * What gets downloaded is the WHOLE ROTATION's images, the post on screen first — at most
 * ten small files, an avatar and a thumbnail for each of five posts. Only the visible post's
 * pair used to be fetched, which was enough while the refresh tick was the only thing that
 * moved the cursor; the reader can now advance the card by hand, and the picture for the post
 * they land on has to already be here, because a tap runs in a broadcast and cannot wait for
 * a download. Rotating is still what makes the widget cheap — the working set is bounded by
 * the rotation rather than by the feed — and [prune] is what keeps that true over months of
 * use rather than accumulating every image the feed has ever shown.
 */
