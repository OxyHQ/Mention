 ...
 *
 *   250 × 110dp  (4×2)  SMALL   brand row, two lines of text, byline. NO image.
 *   250 × 180dp  (4×3)  MEDIUM  the same, plus a short image slot.   (the default)
 *   320 × 320dp  (5×5)  LARGE   more text, and a tall image slot.
 *
...
/**
 * Height of the image slot, per design. `null` where the design has no slot.
 *
 * The slot is a fixed BAND rather than the image's own aspect ratio, because a card
 * whose height moved with whatever picture the rotation landed on would jump every
 * fifteen minutes — and because a portrait photo given its aspect ratio would push the
 * text off a 180dp widget entirely. The image is cropped to fill it.
 */
internal fun imageSlotHeight(size: PostsCardSize): Dp? = when (size) {
    ...
    PostsCardSize.MEDIUM -> 72.dp
    PostsCardSize.LARGE -> 120.dp
}
