...
/**
 * Every image URL the whole rotation references — what the cache DOWNLOADS, and exactly what
 * it is allowed to keep.
 *
 * One set for both, which is what makes the cache directory hold precisely the pictures the
 * widget can currently draw: the refresh worker fetches this list so that a reader advancing
 * the card by hand never lands on a post whose picture is missing, and `prune` evicts
...
/**
 * What TalkBack reads for the card, which is the primary tap target.
 *
...
/**
 * What TalkBack reads for the advance control.
 *
 * It carries the POSITION as well as the action, because the position is otherwise drawn
 * only as pips, and a pip is an untitled image a screen reader has nothing to say about.
 * Announcing it here rather than on the pip row is what keeps it to one utterance: the pips
 * and this button are the same fact twice over, and the button is the half that can be
 * focused and pressed.
 *
 * One-based, as read out: "showing 1 of 5" is where a reader starts, not "0 of 5".
 */
internal fun advanceContentDescription(context: Context, rotation: PostsRotation): String =
    context.getString(
        R.string.mention_posts_widget_next,
        rotation.position + 1,
        rotation.posts.size,
    )
