...
const ComposeScreenBody = ({ presentation }: Required<ComposeScreenProps>) => {
  ...
  //
  // The TAB instance parses nothing. It is a blank composer that stays mounted
  // for the life of the app, so there is no "route entry" for it to lock to —
  // and every intent (a reply, a quote, an edit, a share) is carried by a push
  ...
  //
  // The TAB instance never reads it, and that is load-bearing rather than tidy:
  // this is a one-shot consume, the tab is mounted for the life of the app, and
  // `lib/shareIntent.native.ts` pushes `/compose` — so a tab that read it would
  // empty the buffer on first mount and the composer the share sheet actually
  // opened would find nothing.
  ...
  if (pendingShareMediaRef.current === null) {
    pendingShareMediaRef.current = presentation === 'tab' ? [] : consumePendingShareMedia();
  }
...
const ComposeScreen = ({ presentation = 'pushed' }: ComposeScreenProps) => {
  ...
  return <ComposeScreenBody presentation={presentation} />;
};
