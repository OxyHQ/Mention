...
/**
 * Content federation to remote followers — building the Notes / boosts / likes /
 * `Delete` / `Update` and handing them to the shared delivery engine.
 *
 ...
 * remains reachable from PostCreationService via the connector registry's
 * registered PostFederator (the AP connector's `deliver`).
 */
export class FollowService {
  ...
  async federateBoost(
    ...
    try {
      ...
      await deliveryService.deliverToFollowers(activity, boosterOxyUserId, boosterUsername, {
