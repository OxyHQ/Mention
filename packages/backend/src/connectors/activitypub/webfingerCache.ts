 ...
 * truth shared by `the engine webfinger router (routes/engine.routes.ts)` (which reads/writes it) and
 ...
 * gate runs (see `the engine webfinger router (routes/engine.routes.ts)`), so a toggle in EITHER direction
