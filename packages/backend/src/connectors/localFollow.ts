...
/** Oxy account ids: a UUID (v7 today) or a legacy 24-hex ObjectId. */
const OXY_USER_ID_RE = /^(?:[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}|[0-9a-f]{24})$/i;
 ...
 * federated path owns.
 *
 * - `6981c9…` / `01a0d834-…`       an Oxy account id
...
export function parseLocalFollowRef(raw: string): LocalFollowRef | null {
  ...
  if (OXY_USER_ID_RE.test(value)) return { kind: 'id', id: value.toLowerCase() };
  ...
  if (isAtprotoHandle(bare)) return null;
  return LOCAL_USERNAME_RE.test(bare) ? { kind: 'username', username: bare } : null;
}
...
export async function resolveLocalFollowTarget(ref: LocalFollowRef): Promise<LocalFollowTarget | null> {
  ...
  try {
    const user = ref.kind === 'id'
      ? await oxy.getUserById(ref.id)
      : await oxy.getProfileByUsername(ref.username, { cache: false });
    if (!user?.id) return null;
    return {
      id: user.id,
      username: user.username,
      federated: (user as { type?: string }).type === 'federated',
    };
  } catch {
    return null;
  }
