 ...
 * literal `laneId` term is also what lets the PARTIAL index be used at all. The
 * scope is `oxyUserId` — a lane's posts and its publisher's posts are the same
 * set by construction (`assertLaneAssignable` refuses any other pairing), so the
 * scope term is a narrowing, not a second source of truth.
 *
...
export const laneSource: SourceModule = {
  ...
  gather: async (ctx, params, cap) => {
    ...
    const lane = await Lane.findById(laneId)
      .select('ownerId displayMode')
      .lean<{ ownerId: string; displayMode: LaneDisplayMode } | null>();
    if (!lane) return [];
