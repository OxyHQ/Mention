 ...
 * back-to-back posts from the same author so the stream stays varied (the
 * "connected vs unconnected" balance Instagram uses for Reels chaining).
 */
  ...
  async fetch(options: FeedFetchOptions, context: FeedContext): Promise<FeedAPIResponse> {
    ...
    if (postsToReturn.length > 0 && hasMore) {
      ...
      if (!didCursorAdvance(nextCursor, cursor)) {
        logger.warn('[VideosFeed] Cursor did not advance', { cursor, nextCursor });
        nextCursor = undefined;
      }
    }

    // Thread slicing keeps multi-post video threads grouped; reply context off
    // (reels are standalone clips, not conversation threads).
