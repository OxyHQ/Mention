 ...
 *
 * Postgres has no TTL index. Several Mention models relied on one, so every
 * table that needs it adds an entry here rather than growing its own cleanup
 * path. The registry stays here because it names THIS schema's own tables; the
 * mechanism that sweeps it (`sweepExpiredRows`, `sweepAllExpiredRows`,
 * `ExpirySweepTarget`) lives in `@oxyhq/db/expiry` — see that module's doc
 * comment for the full shape and for why a TTL index is a behaviour of the
 * SOURCE that does not survive a Mongo-to-Postgres port on its own.
 *
