 ...
 * `restrictedUserIds` into hydration, so a mock carrying only the exclusion set
 * makes `getFeed` throw before the exclusion pass ever runs.
 */
function privacyStateExcluding(ids: readonly string[]): MockPrivacyState {
  return {
    blockedUserIds: new Set<string>(),
    mutedUserIds: new Set<string>(ids),
    restrictedUserIds: new Set<string>(),
    excludedUserIds: new Set<string>(ids),
  };
}
...
function excludes(...ids: string[]): void {
  privacy.loadPrivacyState.mockResolvedValue(privacyStateExcluding(ids));
}
...
beforeEach(() => {
  vi.clearAllMocks();
  privacy.loadPrivacyState.mockResolvedValue(privacyStateExcluding([]));
});
