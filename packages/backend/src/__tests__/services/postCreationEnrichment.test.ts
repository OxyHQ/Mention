...
const { resolveDocuments, getUserById } = vi.hoisted(() => ({
...
function warmedUrls(): string[] {
  return resolveDocuments.mock.calls.flatMap(([request]) => request.urls as string[]);
}
...
describe('PostCreationService — post-ingest enrichment', () => {
  ...
  it('creates the post even when the preview service rejects', async () => {
    resolveDocuments.mockRejectedValue(new Error('preview service down'));
