...
async function seedPost(options: SeedPostOptions = {}): Promise<string> {
  const [post] = await db
    ...
    .values({
      ...
      // A parent link forces `is_reply` — `posts_reply_discriminator_check`
      // refuses the disagreeing row, and the live writer derives the flag
      // (`derivesReplyIntent`, `db/posts/postRepository.ts`) rather than
      // letting a caller set one without the other.
      ...(options.parentPostId === undefined
        ? {}
        : { parentPostId: options.parentPostId, isReply: true }),
