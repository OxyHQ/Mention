...
/** A channel row as the write paths read it back. */
...
export async function canPublishToChannel(
  ...
  // Emptiness only. An `ObjectId.isValid` guard stood here and was INERT after
  // the cutover: `channels.id` is `text` holding uuid v7, so it answered `false`
  // for every channel this instance has created — and `false` here means "not a
  // member", which refuses every publish. It failed toward refusal rather than
  // toward exposure, which is why nobody could publish to a channel at all
  // rather than anybody being able to. A `text` id naming no row already matches
  // nothing.
  ...
  const [member] = await getDb()
    ...
    .where(
      and(
        ...
        eq(channelMembers.oxyUserId, oxyUserId),
...
export async function assertCanPublishToChannel(
  ...
  // No id-SHAPE guard, for the reason spelled out on `canPublishToChannel`. An
  // id naming no channel is the same 404 a real-but-unknown one gets, which is
  // what keeps this from being an existence oracle.
...
export async function bumpChannelCounter(
  ...
  if (!channelId || delta === 0) return;
  try {
    // `count + delta` in SQL, never a read-modify-write: this is the direct
    // analogue of Mongo's `$inc` and it is what makes two concurrent publishes
    // add up to two. The `>= 0` CHECK on the table is what a decrement race
    // cannot get past — in Mongo it could take a counter negative and nothing
    // would notice.
    const column = channels[field];
    await getDb()
      .update(channels)
