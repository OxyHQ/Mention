...
- **A 201 from `POST /reports` means stored-and-will-retry, NEVER "CrowdSource accepted it."** `ReportIntakeService` commits the `Report` and its `ModerationOutbox` event in ONE transaction; no outbound request is made in the request handler. Two writes outside one transaction give two silent failure modes (a report nothing will ever send; an event whose report was rolled back) and neither surfaces as an error when it happens. **`enqueueModerationOutboxEvent` throws unless `session.inTransaction()`** — the type makes the session mandatory, the runtime check makes it mandatory that a transaction is actually open, because a bare `startSession()` type-checks, commits the row alone, and passes any test that only asserts the row exists. It is also the ONLY writer of that collection (the dispatcher claims existing rows, never creates one), so no second queue can drift out of sync: the row IS the job.
...
- **Media evidence is declared, not attached.** `AssetRef` requires a `sha256` of the exact bytes, Mention stores none for Oxy file ids or federated URLs, and `POST /v1/uploads` is not served by the CrowdSource backend yet. A post with no text gets a `metadata` subject resource saying what it consisted of, so a jury can answer `insufficient_context` for the right reason. Closing this is one function in `postSubject.ts` plus an upload call.
- **Mention only reports objects Mention OWNS — `post`, `comment`, `user`.** This is a tenancy boundary, not a capability gap, and `subjects/registry.ts` is its single authority (`POST /reports` refuses anything else with 400). `applicationId` comes off the credential, so a report Mention submits opens a case in *Mention's* tenant: a case about a Syra live room could not be enforced here, and when Syra reports the same room under its own credential §7.3's dedup key differs by tenant, so one incident gets two cases, two juries and two consequences. That breaks one-decision-per-incident where nothing inside Mention can repair it. There is deliberately **no local state meaning "stored, and nothing will ever happen to it"** — that would be a receipt for work nobody does, with no queue to alert on. Cross-application hand-off is an open design question; do not "fix" this by widening the enum.
  - `frontend/services/reportService.ts` `reportRoom` is therefore **a live frontend bug**, reachable from the live-rooms overflow menu (`app/(app)/live-rooms/[id].tsx`) for any non-host. It has always received a 400 and shown "Failed to submit report." — an honest failure. Do not make it succeed.
  - `ReportedType.MESSAGE` stays in the enum only because removing an enum value is destructive to stored documents. Nothing in the UI has ever produced one.
