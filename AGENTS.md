...
- **Datastore**: Postgres, and nothing else. `/health/ready` reports `postgres` · `migrations` · `redis` — there is no `mongo` entry, and the task definition carries no `MONGODB_URI` secret. The deploy's pre-rollout one-shots are, in order: **Postgres migration** · **Postgres population floor** · **Blocked-domain reconciliation** (see `.github/scripts/deploy-ecs-image.sh`).
...
## Frontend coverage: `test` is not the gate, and a broken `.web` fork RAISES the number

`bun run test` computes no coverage. CI's frontend leg runs `test:coverage` and then `coverage:check`, which evaluates `packages/frontend/coverage-policy.json` against the report: per-file pins for all 238 behavioural-logic files, an 80%/65% bar on the seven critical paths, a no-regression ratchet, and a refusal to let the committed baseline drop against the base revision. Rationale, the exemption list and what to do when it stops you: `packages/frontend/docs/TESTING-POLICY.md`. After genuinely raising coverage, run `coverage:record` and commit `packages/frontend/coverage-baseline.json` in the same change.

**`jest-expo` resolves the NATIVE platform extension**, so a bare relative import never loads the sibling web fork; a test whose specifier spells the `.web` suffix out DOES load it, which is why three of this package's sixteen web forks carry real coverage and the rest are listed `unenforceable`. Measured: a syntax error in `lib/socketBfcache.web.ts` left `test:coverage` at exit 0 with 165/165 suites passing, printed only `Failed to collect coverage from …`, and pushed overall statements UP from 28.34% to 28.35% because the file left the denominator. Every percentage moves the reassuring way; the only honest signal is the file's ABSENCE from the report, which is what the policy asserts.
...
## PostgreSQL — the only store, and what the port left behind

**Mention was on MongoDB until August 2026 and is now Postgres-only.** Nothing in this repository can open Mongo: the copier, the Mongoose models, the index manifest, the `mongoose`/`mongodb-memory-server` dependencies and every `MONGODB_*` config binding are deleted, so no such variable is declared, read, or required to boot. The SSM parameter `/oxy/mention/MONGODB_URI` has been deleted, and oxy-infra's terraform records `DATABASE_URL` for this service, so nothing upstream can put the secret back on a task definition either. The deploy therefore carries no standing directive to strip it — the in-repo assertion that Mongo stays gone is `scripts/validate-no-mongo.mjs`, run by `bun run check` and enforced in CI. History, including the cutover runbook: `docs/MONGO-TO-POSTGRES-CUTOVER.md`, itself now a short note over git history.

- **Per-table conventions are `packages/backend/src/db/schema/CONVENTIONS.md`** — the binding document for adding a table or a column. `packages/backend/src/db/MIGRATION-CONTRACT.md` is a historical stub holding the two RDS facts below; ecosystem-wide porting rules live in oxy-api's own migration contract, in the OxyHQServices repo — not a path in this one, which is why it is named rather than linked.
...
- **Ported ids are 24-hex, new ids are uuid v7, and the two interleave under text collation.** `posts.id` and friends are `text`, so `id < X` is not a time bound and `ORDER BY id DESC` is not a time order. Never page or order a chronological query by id — `mtn/feed/CursorBuilder.ts` carries the keyset that replaced it.
- **Local dev is two compose files, on purpose.** `docker-compose.yml` runs the whole stack (PostGIS + Valkey + a migrate one-shot + the backend image) on loopback port 5434; `docker-compose.postgres.yml` binds 5433 for the database that sits beside `bun run dev:backend` and the test suite, so both can be up at once. `server.ts` opens Postgres unconditionally at boot and `/health/ready` never turns ready without it, so `DATABASE_URL` is required — `packages/backend/.env.example` has the value that pairs with 5433.
...
packages/
  ...
  backend/        @mention/backend     Express / Postgres (Drizzle) / Redis / Socket.io
...
- **Backend**: Express, Postgres via Drizzle ORM, Redis, Socket.io, Firebase Admin, Oxy media services. There is no `mongoose` dependency and no Mongo of any kind.

## MTN Protocol (Mention's signed-records layer)

Mention posts are dual-written as signed records on a per-user hash chain — the "MTN Protocol" — riding on the shared `@oxyhq/protocol` engine. Postgres stays authoritative for the post itself; the chain write is a best-effort SIDE-EFFECT, isolated (`Promise.allSettled`), and gated on local authors (`federation == null && oxyUserId`).
...
- **`mention_signed_records` / `mention_repo_heads`** — the chain's storage, ported to Postgres tables (`db/schema/mtn.ts`), keyed by `oxyUserId`. `MentionRecordStore` is the `@oxyhq/protocol` `RecordStore` adapter over them (the Mongoose `MentionSignedRecord`/`MentionRepoHead` models these replaced are gone). The append is one `db.transaction`, unconditionally — the old Mongoose version's session-less fallback for a standalone `mongod` had no Postgres equivalent to port.
...
- **`NetworkConnector` interface + normalized DTOs** (`NormalizedExternalActor`, `NormalizedExternalPost`, `LocalNetworkEvent`) — no longer a local connectors/types.ts file; they now ship from the shared `@oxyhq/federation` package, which is the layer having actually been extracted rather than merely kept free of a local ORM coupling.
...
- **One-shot scripts in `src/scripts/` MUST close every resource they may have opened and `process.exit()` when done** — imported singletons (Postgres pool, BullMQ Redis connections, MediaCache workers) otherwise keep the Fargate one-shot task running forever. `closeAdminScriptResources()` (`scripts/lib/adminScriptLifecycle.ts`) is the shared helper (Postgres, BullMQ queues, Redis) and is safe to call even when nothing connected.
...
- **The human who wrote it is `Post.writtenByOxyUserId`, never in `authorship[]`.** Putting them in `authorship` would both break the channel's anonymity and put the post back on their own profile and their followers' timelines. `__tests__/channelAccountSchema.test.ts` mutation-tests exactly that against REAL ROWS and asserts the damage — and the damage is no longer latent: the `channel_id is null` exclusion that used to hide it was dropped with the column by `0017_a_channel_is_an_account`, so the authorship placement is now the only thing holding the property up.
...
- **Managing a channel's post answers to CURRENT membership, never to `writtenByOxyUserId`.** `services/postManagementAccess.ts`'s `postManagementRefusal` is the one gate for all seven management routes (edit, edit-source, settings, lane move, delete, publish-early, insights) and has NO per-route opt-out — the stored writer column is written once at creation and never revised, so honouring it let a removed member keep destroying, rewriting, pinning and measuring the channel's work. Only "the caller IS the authoring account" is free. **The DTO's `viewerState.isOwner` deliberately still reads the writer column and therefore diverges**: a departed writer is drawn a menu every route 404s. Do not "fix" that by narrowing `isOwner` — a channel authors its own posts, so the writer clause is the only thing giving any human a menu on a channel post outside the editorial queue.
...
- **A channel post has NO 30-minute edit window — it has a correction trail instead, and the trail is the half that matters.** A publication is expected to fix what it published however long ago, so `updatePost` skips the window for a `channel` author (`isChannelAccount`, the same identity read the reply gate uses). That is only acceptable BECAUSE every change to a published channel post's body appends a `post_corrections` row holding what it said before: permanent editability without the trail is strictly worse than the window, since it lets a publication silently rewrite what people read. Ship the trail or neither.
  - **Three conditions, each excluding a case with nothing to be accountable for:** a channel author, a change to the BODY (a pin, a lane move or a media swap is not a correction, and `isEdited` never counted one either), and a post that was already `published` before the edit (a draft or scheduled post has no readers). A personal post keeps its window and records nothing — that rule exists for a different reason and is deliberately untouched.
  - **`isChannelAccount` fails soft to `false`, which here means the window APPLIES.** During an identity outage a late correction is refused rather than allowed; refusing an edit is the recoverable direction.
  - **The trail carries NO author, though it stores one.** `post_corrections.corrected_by_oxy_user_id` is written for audit and never selected by `listPostCorrections` — a correction is made by exactly the writer `signPosts` exists to keep undisclosed, so serving it would route around that setting. Same reasoning as `posts.written_by_oxy_user_id`, one table over.
  - **`posts.correction_count` counts corrections MADE, never rows retained**, and is the source of the revision number (`update … returning` in one statement, so two concurrent corrections cannot both claim revision N). Retention keeps revision 1 — the post AS PUBLISHED — plus the most recent `MAX_RETAINED_POST_CORRECTIONS - 1`; a gap in the surviving `revision` numbers is where an intermediate body was dropped. An aggregate over the table would shrink as retention bit and start reissuing revision numbers.
  - **Nothing changes in federation or on the MTN chain, and neither needs to.** The existing `Update(Note)` with its `updated` stamp is the only interoperable edit signal — do NOT invent an AS2 correction extension nobody honours. The chain already IS an append-only signed trail: `emitPostCreated` re-emits under the SAME rkey and `mention_signed_records` has no unique index on `(oxy_user_id, nsid, rkey)`, so every version persists, signed, ordered by `seq`, LWW-materialized to the newest. It corroborates the trail; it does not serve it (best-effort, and sparse in production).
  - `GET /posts/:id/corrections` is PUBLIC and reuses the post's own ACL by hydrating it and 404ing when hydration drops it — never a second implementation of "may this viewer see this post".
...
- **Postgres removes six direct-FK children (`polls`, `articles`, `likes`, `bookmarks`, `post_recent_repliers`, `engagement_outbox`, plus seven more nothing probes) inside the SAME `DELETE` statement via `ON DELETE CASCADE`** — writing an explicit leg for those would re-implement work the database already does, and it would be permanently untestable (the residue check runs after the delete, when the FK-cascaded rows are already gone either way, so it cannot tell "my leg ran" from "the FK ran"). `PostDeletionCascade.ts` handles explicitly only the seven references a foreign key cannot express: polymorphic (`notifications.entity_id`, `reports.reported_id`, `content_labels.target_id`), URI-keyed (`feed_interactions.post_uri`), a JSON blob (`federation_delivery_queue.activity_json`), and two gate tables whose `post_id` is plain `text()` because the gate is upserted on `post_uri` without proving the post exists. Do not "complete" the module by adding the FK-covered six back. `PostSubscription` (keyed `{subscriberId, authorId}`, no `postId` column at all) is excluded from this cascade entirely — `deletePost` calling `PostSubscription.deleteMany({ postId })` has always matched zero rows, dead code carried over from the Mongo original rather than a relation; author subscriptions are cleaned by the ACTOR-deletion cascade instead, which probes its two real fields.
...
- **Spacer size:** use `Math.max(totalSize, lastItemEnd)` (`virtualItems.at(-1)?.end ?? 0`). Keep it — but it is belt-and-braces, and the story it used to carry ("on prod builds `getTotalSize()` can return 0 even with measured rows") was NOT a react-virtual quirk, so don't repeat that. It was the **React Compiler**, and the mechanism generalizes to every virtualized web list here.

  `useWindowVirtualizer` returns an instance whose identity is stable for the component's lifetime (`useState(() => new Virtualizer(...))`) and forces re-renders through a reducer INTERNAL to the hook. Nothing the component can see changes on scroll, so the compiler caches `getTotalSize()` — and, when its grouping reaches that far, `getVirtualItems()` and the whole row JSX — keyed on props plus that stable instance, and serves the first result forever. Dev hides it (`enableResetCacheOnSourceFileChanges` is true there, so fast refresh clears the cache on every edit); prod does not.

  Verified by compiling `Feed.web.tsx` as of `e4ea113f`, the commit that added the `Math.max`: it compiled, with `getTotalSize()` frozen on `virtualizer`. `ce87804e` an hour later added a render-phase ref write that made the compiler refuse the whole function, removing the freeze **by accident** — which is why `Math.max` has been redundant since. Do not remove it; a workaround whose cause is gone is still cheap insurance.

  **A virtualized web list must therefore be opted out explicitly.** `ProfileGridList.web.tsx` and `SavedPostsList.web.tsx` carry `'use no memo'` with the reason in-source; `Feed.web.tsx` and `NotificationsList.web.tsx` are opted out only ACCIDENTALLY, by reading a ref during render — `Feed.web.tsx` carries a comment saying so, because removing those reads reintroduces the freeze in the most-used screen in the app. Symptoms differ by how far the grouping reached: a frozen `getTotalSize()` alone means a short spacer and sticky rails that scroll away; a frozen `getVirtualItems()` means rows past the first window never mount (`ProfileGridList` shipped that way — 8 rows and ~550px of blank below them).

  Do NOT reason about which shape is safe: grouping is per-file, and a captured ref does NOT reliably prevent it (measured both ways). Compile the file with the app's own `babel-plugin-react-compiler` and read the CompileError/CompileSuccess **events** — not a grep of the output, which comments like these will match. Always verify virtualization bugs on a PROD build (`expo export web`), not the dev server.
...
**Search is now Postgres full-text, not a MongoDB text index.** `postContentVariants.searchVector` (`db/schema/postContent.ts`) is a generated `tsvector` column, `to_tsvector('english', coalesce(body, ''))`, queried with `websearch_to_tsquery('english', ...)` in `routes/search.ts`. It is unconditionally English — there is no per-document `language_override` mechanism to get wrong, which is a simpler way to land on the same practical behavior the old Mongo index's sentinel field produced (always English stemming). The retired Mongo rule — never let a text index's `language_override` point at a field holding free-form content-language codes, or MongoDB rejects the write with error 17262 — no longer applies to anything at all: the model that declared the `content.text_text` index has been deleted.
...
- **A 201 from `POST /reports` means stored — and will-retry when the type is deliverable — NEVER "CrowdSource accepted it."** `ReportIntakeService` commits the `Report` and its `moderation_outbox` row in ONE Postgres transaction (`db.transaction`); no outbound request is made in the request handler. Whether a delivery event exists at all is decided from ONE fact (a subject provider) read before the transaction body, so `localStatus` and the outbox row can never disagree. Two writes outside one transaction give two silent failure modes (a report nothing will ever send; an event whose report was rolled back) and neither surfaces as an error when it happens. This moderation subsystem is on Postgres (`db/schema/moderation.ts`), not Mongo.
- **`enqueueModerationOutboxEvent` refuses the ROOT connection, not just a missing session.** The old Mongo invariant was `session.inTransaction()`: a type made a session mandatory, a runtime check made it mandatory that a transaction was actually OPEN, because a bare `startSession()` type-checks and commits the row alone. Drizzle's `DatabaseOrTransaction` param has the same hole and a wider one — the ROOT `Database` satisfies the type too, and every repository here defaults to it, so forgetting the argument IS the mistake. `requireTransaction()` (`db/moderation/transactionGuard.ts`) discriminates by capability, not by name: the root database has no `rollback`, a transaction (or nested savepoint) handle does, and it throws `MissingTransactionError` when handed the former. `enqueueModerationOutboxEvent` is also the ONLY inserter into `moderation_outbox` (the dispatcher claims existing rows, never creates one), so no second queue can drift out of sync: the row IS the job.
- **The upsert is `.onConflictDoNothing({ target: moderationOutbox.id })`, and a repeated enqueue is a genuine no-op for a different structural reason than the Mongo version worked to achieve.** The Mongo write was `{ upsert: true, session, timestamps: false }` with `createdAt`/`updatedAt` explicit inside `$setOnInsert`, because `ModerationOutbox` declared `{ timestamps: true }` and letting Mongoose add its own `updatedAt` on top named that path twice in one update — Mongo refused the WHOLE write, taking the `Report` down with it. Drizzle has no implicit timestamping on a conflict branch: `$onUpdate` fires only for an `update()`, and `onConflictDoNothing` writes nothing at all on a duplicate id, so there is no second `updatedAt` writer to collide with.
...
- `moderationOutboxDispatcher` starts on EVERY task (`server.ts`, next to `engagementOutboxDispatcher`): claims are `SELECT ... FOR UPDATE SKIP LOCKED` over Postgres (ported from Mongo's atomic `findOneAndUpdate` over a disjunctive filter — Postgres row locking is the better primitive for the same job), so N tasks share the work and a dead task's expired lease is reclaimed without dispatchers handing each other the same row. No-ops when `CROWDSOURCE_ENABLED=false` — the LOOP is gated, never the durable record, so reports taken while off deliver when it is switched on.
...
- The webhook dedupe store is **Postgres-backed** (`services/moderation/moderationEventStore.ts` over `db/moderation/moderationEventRepository.ts` — ported off Mongo, not Mongo-backed anymore) because Mention runs several ECS tasks; the SDK's in-process default would dedupe only the instance that received both copies of a redelivery. `moderation_events.id` IS the webhook event id — the unique primary key is the dedupe.
