...
- **Datastore**: production is fully on Postgres. `/health/ready` reports `postgres` · `migrations` · `redis` — there is no `mongo` entry, and the task definition carries no `MONGODB_URI` secret. The deploy's pre-rollout one-shots are, in order: **Postgres migration** · **Postgres population floor** · **Blocked-domain reconciliation** (see `.github/scripts/deploy-ecs-image.sh`) — the old Mongo migration entry is gone. See § MongoDB → PostgreSQL Migration below for what still touches Mongo and what does not.
...
**Production has cut over.** Binding contract: `packages/backend/src/db/MIGRATION-CONTRACT.md` (Mention's deltas + per-table conventions in `packages/backend/src/db/schema/CONVENTIONS.md`). Ecosystem-wide rules live in oxy-api's own migration contract, in the OxyHQServices repo — not a path in this one, which is why it is named rather than linked.
...
- **What still touches Mongo, on purpose:** `packages/backend/scripts/backfill-mongo-to-postgres.ts` + `src/db/backfill/mongoSource.ts` (the copier used to rebuild Postgres from Mongo) are the ONE surviving consumer, which is why `mongoose` stays a `packages/backend` dependency. The SSM parameter `/oxy/mention/MONGODB_URI` still exists but the deploy no longer syncs it from GitHub Actions — if the Mongo endpoint or its credentials ever rotate, that parameter needs updating BY HAND, or the copier silently points at a dead source.
- **Local dev has NOT been ported and currently needs BOTH stacks.** `docker-compose.yml` still defines only `mongo` + `mongo-init` (no Postgres service) and `packages/backend/.env.example` still only documents `MONGODB_URI` — neither has caught up with production. A developer also needs `docker-compose.postgres.yml` (a separate compose file, `docker compose -f docker-compose.postgres.yml up -d postgres`) and a hand-set `DATABASE_URL` pointing at it, because `server.ts` opens Postgres unconditionally at boot and `/health/ready` never turns ready without it. Do not read the Mongo-only `docker-compose.yml`/`.env.example` as evidence the app still runs on Mongo — it does not; those files are just stale.
...
- **Backend**: Express, Postgres via Drizzle ORM, Redis, Socket.io, Firebase Admin, Oxy media services. `mongoose` is still a dependency with exactly one consumer: `packages/backend/scripts/backfill-mongo-to-postgres.ts` (the Mongo→Postgres copier — see § MongoDB → PostgreSQL Migration). Production carries no `MONGODB_URI` and boots without it.
