...
- **Nothing touches Mongo any more.** The copier that rebuilt Postgres from Mongo, the Mongoose models, the index manifest and the `mongoose`/`mongodb-memory-server` dependencies are all deleted, so there is no code path left that could open it and no `MONGODB_*` variable the config will even parse. The SSM parameter `/oxy/mention/MONGODB_URI` still exists and is now read by nothing.
...
- **Backend**: Express, Postgres via Drizzle ORM, Redis, Socket.io, Firebase Admin, Oxy media services. There is no `mongoose` dependency and no Mongo of any kind — see § MongoDB → PostgreSQL Migration.
