...
- **`isAuthResolved` gates auth-dependent work.** Use `isAuthResolved` (from `@oxyhq/services ^8.5.0`) to hide/skeleton auth-dependent affordances until cold boot is done. Do NOT gate public feed-content fetches on it — that deferred `getMtnFeed` and stranded the authed feed on a spinner. DO gate private endpoints (`/managed-accounts`, privacy lists, follow-status mutations, profile/settings) on `isAuthResolved && isAuthenticated` plus an available access token.
