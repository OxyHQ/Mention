...
- **A 201 from `POST /reports` means stored-and-will-retry, NEVER "CrowdSource accepted it."** `ReportIntakeService` commits the `Report` and its `ModerationOutbox` event in ONE transaction; no outbound request is made in the request handler. Two writes outside one transaction give two silent failure modes (a report nothing will ever send; an event whose report was rolled back) and neither surfaces as an error when it happens. `enqueueModerationOutboxEvent` REQUIRES a `ClientSession` for exactly this reason — never make it optional.
