...
- **A thread REFUSES a per-post account (400); a `beast` batch honours one per entry.** Not a style call: thread continuations are replies, and publishing a reply as another account is refused outright, so applying the root's identity downward needs a hole in that rule — and a channel would refuse its own thread's continuations anyway. The batch-level field is refused in both modes, naming the per-entry one, because two ways to say it lets a client send both and discover which won.
