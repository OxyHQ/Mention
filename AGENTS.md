...
- **Anonymity is STRUCTURAL, not a render-time decision** — the writer simply has no DTO field today, so `UserSettings.channel.signPosts` (keyed on the CHANNEL's `oxyUserId`) currently gates nothing visible. Disclosure needs a wire field first; until then the toggle is the seam, not the behaviour.
